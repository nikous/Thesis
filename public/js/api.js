...
async function getData(symbol) {
  ...
  await $.ajax({
    ...
    success: function (data) {
      ...
      var endMonth = date_labels.findIndex(
        (element) =>
          element.includes(today.getFullYear()) && element.endsWith(curMonth, 7)
      );

      // If month doesn't exist go to previous month
      if (endMonth == -1) {
        // Find index of previous month in current year ---> END
        endMonth = date_labels.findIndex(
          (element) =>
            element.includes(today.getFullYear()) &&
            element.endsWith(prvMonth, 7)
        );
      }

      // Find index of current month in previous year ---> START
      var startMonth = date_labels.findIndex(
        (element) =>
          element.includes(today.getFullYear() - 1) &&
          element.endsWith(curMonth, 7)
      );
...
async function getDataReal(symbol) {
  ...
  await $.ajax({
    ...
    success: function (data) {
      ...
      // If it's Monday and hour is bigger than 16:35 so data for Monday,friday thursday
      if (numDay == 1 && numHour >= 16) {
        dateStock = 4;
      }
      ...
      var secIndex = date_array_Real.findIndex((element) =>
        element.endsWith('16:00:00')
      ); // End hour

      var openIndex = date_array_Real.findIndex((element) =>
        element.endsWith('09:45:00')
      ); // Start hour
      ...
      document.getElementById('open').innerHTML =
        ' ' + open_array_Real[openIndex];
      document.getElementById('high').innerHTML =
        ' ' + high_labels_1Day[indexOfMax(close_labels_3Days_chart)];
      document.getElementById('low').innerHTML =
        ' ' + low_labels_1Day[indexOfMin(close_labels_3Days_chart)];
      document.getElementById('PrevClose').innerHTML =
        ' ' + close_array_Real[secIndex];
      ...
      if (close_array_Real[getIndex] >= close_array_Real[secIndex]) {
        document.getElementById('diference').innerHTML =
          ' +' +
